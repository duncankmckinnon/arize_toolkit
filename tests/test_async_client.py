...
from arize_toolkit.async_client import AsyncClient
from arize_toolkit.queries.basequery import ArizeAPIException


@pytest.fixture
def mock_transport():
    """Create a mock AIOHTTPTransport"""
    with patch("arize_toolkit.async_client.AIOHTTPTransport") as mock_transport:
        # Mock async context manager methods
        mock_transport.return_value.__aenter__ = AsyncMock(return_value=mock_transport.return_value)
...
def mock_graphql_client():
    """Create a mock async GraphQL client"""
    with patch("arize_toolkit.async_client.GraphQLClient") as mock_client:
        # Create async mock for execute_async
        mock_client.return_value.execute_async = AsyncMock()

        # Mock the initial org/space lookup response
        mock_client.return_value.execute_async.return_value = {
            "account": {
                "organizations": {
                    "edges": [
                        {
...
@pytest.fixture
def client_factory(mock_graphql_client, mock_transport):
    """Factory to create async client instances for tests"""
...
class TestAsyncModel:
    @pytest.mark.asyncio
    async def test_get_model_by_id(self, client_factory, mock_graphql_client):
        """Test getting a model by ID"""
        mock_graphql_client.return_value.execute_async.reset_mock()

        # Mock responses: initial setup + model query
        mock_responses = [
            # Initial org/space lookup
            {
                "account": {
                    "organizations": {
                        "edges": [
        ...
        mock_graphql_client.return_value.execute_async.side_effect = mock_responses

        async with client_factory() as client:
            result = await client.get_model_by_id("test_model_id")
    ...
    @pytest.mark.asyncio
    async def test_get_model(self, client_factory, mock_graphql_client):
        ...
        async with client_factory() as client:
            result = await client.get_model("test_model")
    ...
    @pytest.mark.asyncio
    async def test_get_model_not_found(self, client_factory, mock_graphql_client):
        """Test model not found error"""
        mock_graphql_client.return_value.execute_async.reset_mock()

        # Mock responses: initial setup + empty model response
        mock_responses = [
            # Initial org/space lookup
            {
                "account": {
        ...
        async with client_factory() as client:
            with pytest.raises(ArizeAPIException) as exc_info:
                await client.get_model("non_existent_model")
            assert "No model found" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_get_all_models(self, client_factory, mock_graphql_client):
        ...
        async with client_factory() as client:
            results = await client.get_all_models()
    ...
    @pytest.mark.asyncio
    async def test_get_model_volume(self, client_factory, mock_graphql_client):
        """Test getting model volume by name"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        async with client_factory() as client:
            result = await client.get_model_volume(model_name="test_model")

            assert mock_graphql_client.return_value.execute_async.call_count == 3
            assert result == 200

    @pytest.mark.asyncio
    async def test_get_total_volume(self, client_factory, mock_graphql_client):
        """Test getting total volume for all models"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        async with client_factory() as client:
            total_volume, model_volumes = await client.get_total_volume()

            assert total_volume == 300
            assert model_volumes["model1"] == 100
            assert model_volumes["model2"] == 200
            assert mock_graphql_client.return_value.execute_async.call_count == 4

    @pytest.mark.asyncio
    async def test_delete_data_by_id(self, client_factory, mock_graphql_client):
        """Test deleting model data by ID"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        async with client_factory() as client:
            result = await client.delete_data_by_id("test_model_id", "2021-01-01T00:00:00Z")
            assert result

    @pytest.mark.asyncio
    async def test_delete_data(self, client_factory, mock_graphql_client):
        """Test deleting model data by name"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        async with client_factory() as client:
            result = await client.delete_data("test_model", "2021-01-01", "2021-01-02", "PREPRODUCTION")
            assert result
            assert mock_graphql_client.return_value.execute_async.call_count == 3


class TestAsyncCustomMetrics:
    @pytest.mark.asyncio
    async def test_get_all_custom_metrics(self, client_factory, mock_graphql_client):
        """Test getting all custom metrics for a model"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        # Mock response for get_all_custom_metrics with model_name
        mock_responses = [
            # Initial org/space lookup
            {
                "account": {
        ...
        async with client_factory() as client:
            results = await client.get_all_custom_metrics(model_name="test_model")
    ...
    @pytest.mark.asyncio
    async def test_create_custom_metric(self, client_factory, mock_graphql_client):
        ...
        async with client_factory() as client:
            url = await client.create_custom_metric(
                metric="SELECT AVG(accuracy) FROM model",
                metric_name="accuracy",
                model_name="test_model",
                metric_description="Model accuracy metric",
            )

            expected_url = client.custom_metric_url("test_model_id", "new_custom_metric_id")
            assert url == expected_url
            assert mock_graphql_client.return_value.execute_async.call_count == 3

    @pytest.mark.asyncio
    async def test_get_custom_metric(self, client_factory, mock_graphql_client):
        """Test getting a custom metric by name and model"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        mock_responses = [
            ...
            # Get custom metric response (this is the direct query, not nested under model lookup)
            {
                "node": {
                    "models": {
                        "edges": [
        ...
        async with client_factory() as client:
            result = await client.get_custom_metric("test_model", "accuracy")

            assert result["id"] == "custom_metric_id_1"
            assert result["name"] == "accuracy"
            assert result["description"] == "Model accuracy"
            assert mock_graphql_client.return_value.execute_async.call_count == 2


class TestAsyncMonitors:
    @pytest.mark.asyncio
    async def test_get_all_monitors(self, client_factory, mock_graphql_client):
        """Test getting all monitors for a model"""
        mock_graphql_client.return_value.execute_async.reset_mock()
        ...
        async with client_factory() as client:
            # Test with model_id
            results = await client.get_all_monitors(model_id="test_model_id")
    ...
    # Note: Monitor creation methods (create_performance_monitor, create_drift_monitor, etc.)
    # are complex and not yet implemented in AsyncClient
    # TODO: Add async versions of monitor creation methods
...
class TestDashboards:
    ...
    def test_get_dashboard_url_simple_client(self, mock_graphql_client):
        client = AsyncClient(organization="test_org", space="test_space")
