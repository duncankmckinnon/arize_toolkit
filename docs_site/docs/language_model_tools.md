...
url = client.create_prompt(
    name="greeting_prompt",
    messages=[{"role": "system", "content": "You are a helpful assistant."}, {"role": "user", "content": "Hello {user_name}!"}],
)
```
...
formatted = client.get_formatted_prompt("troubleshoot_prompt", question="Why is my widget broken?")
