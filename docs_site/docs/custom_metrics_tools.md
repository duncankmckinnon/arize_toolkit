...
3. Updating or deleting metrics  
4. Copying metrics between models
...
```python
metric = client.get_custom_metric(model_name="fraud-detection-v3", metric_name="is_fraud_like")
print(metric)
```
...
```python
url = client.get_custom_metric_url(model_name="fraud-detection-v3", metric_name="is_fraud_like")
print(url)
```
