...
class BaseQuery:
    ...
    ) -> Tuple[List[QueryResponse], bool, Optional[str]]:
        try:
    ...
    def iterate_over_pages(
        cls, client: GraphQLClient, sleep_time: int = 0, **kwargs
    ) -> List[QueryResponse]:
        result = []
        cursorCount = 100
        ...
        while hasNextPage and cursorCount > 0:
            currentPage, hasNextPage, endCursor = cls._graphql_query(
                client, endCursor=endCursor, **kwargs
            )
            result.extend(currentPage)
            cursorCount -= 1
            sleep(sleep_time)
